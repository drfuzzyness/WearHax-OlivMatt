...
////// Lighting:
                ...
                float3 finalColor = (attenuation*_LightColor0.rgb*((_Diffuse_var.rgb*_Color.rgb*floor(0.5*dot(lightDirection,i.normalDir)+0.5 * _Bands) / (_Bands - 1))+UNITY_LIGHTMODEL_AMBIENT.rgb+floor(pow(max(0,dot(i.normalDir,halfDirection)),exp2(((_Gloss*10.0)+0.0))) * _Bands) / (_Bands - 1)));
